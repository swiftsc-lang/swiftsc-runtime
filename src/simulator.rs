...
impl ContractSimulator {
    pub fn new() -> Self {
        ...
        // Host functions
        linker.func_wrap("env", "get_caller", |caller: Caller<'_, ContractState>| -> i64 {
            caller.data().caller
        }).expect("failed to wrap get_caller");

        linker.func_wrap("env", "get_value", |caller: Caller<'_, ContractState>| -> i64 {
            caller.data().value
        }).expect("failed to wrap get_value");

        linker.func_wrap("env", "get_data", |caller: Caller<'_, ContractState>| -> i64 {
            caller.data().data
        }).expect("failed to wrap get_data");

        linker.func_wrap("env", "storage_read", |caller: Caller<'_, ContractState>, key: i64| -> i64 {
            *caller.data().storage.get(&key).unwrap_or(&0)
        }).expect("failed to wrap storage_read");

        linker.func_wrap("env", "storage_write", |mut caller: Caller<'_, ContractState>, key: i64, value: i64| {
            caller.data_mut().storage.insert(key, value);
        }).expect("failed to wrap storage_write");

        linker.func_wrap("env", "emit_event", |mut caller: Caller<'_, ContractState>, id: i64, data: i64| {
            println!("EVENT EMITTED: id={}, data={}", id, data);
            caller.data_mut().events.push((id, data));
        }).expect("failed to wrap emit_event");

        linker.func_wrap("env", "hash_i64", |_: Caller<'_, ContractState>, a: i64, b: i64| -> i64 {
            // A robust non-cryptographic hash (splitmix64 style)
            let mut x = (a as u64).wrapping_add(b as u64).wrapping_add(0x9E3779B97F4A7C15);
    ...
    pub fn run(&self, wasm_bytes: &[u8], func_name: &str, args: &[i64], state: ContractState) -> Result<(i64, ContractState)> {
        let module = Module::new(&self.engine, wasm_bytes).map_err(|e| {
            let hex = wasm_bytes.iter().map(|b| format!("{:02x}", b)).collect::<Vec<String>>().join("");
            let _ = std::fs::write("/tmp/contract.wasm.hex", &hex);
            anyhow!("Failed to load module: {}\nWASM hex saved to /tmp/contract.wasm.hex", e)
        })?;
        ...
        let func = instance.get_export(&store, func_name)
        ...
        let res = if let Some(wasmi::Value::I64(val)) = results.get(0) {
            *val
        } else {
            0
        };
